<?
$MESS['REVO_OPTIONS_TAB'] = 'Основные';
$MESS['REVO_OPTIONS_TITLE'] = 'Основные настройки';
$MESS['API_OPTIONS_TAB'] = 'API';
$MESS['API_OPTIONS_TITLE'] = 'Настройки API';
$MESS['OPTIONS_ORDERS_URL'] = 'Страница с заказами';
$MESS['OPTIONS_DETAIL_MIN_PRICE'] = 'Минимальная цена товара для отображения виджета';
$MESS['OPTIONS_DETAIL_MAX_PRICE'] = 'Максимальная цена товара для отображения виджета';
$MESS['OPTIONS_RETURN_STATUS_CODE'] = 'Код статуса, при котором отправлять отмену заказа';
$MESS['OPTIONS_FINAL_STATUS_CODE'] = 'Код статуса, при котором отправлять финализацию заказа';
$MESS['OPTIONS_DETAIL_MAX_ORDER_PART'] = 'Максимальная доля заказа для оплаты частями, %';
$MESS['OPTIONS_DETAIL_MAX_ORDER_PART_MIN'] = 'Доля заказа для оплаты частями должна быть от 20% до 100%';
$MESS['OPTIONS_ADMIN_EMAIL'] = 'Email администратора';
$MESS['OPTION_INSTALMENT_PERIOD'] = 'Максимальное кол-во месяцев по тарифу';
$MESS['OPTIONS_JQUERY_SELECTOR'] = 'Включить JQuery';
$MESS['OPTIONS_API_URL'] = 'Боевой URL';
$MESS['OPTIONS_API_TEST_URL'] = 'Тестовый URL';
$MESS['OPTIONS_API_METCHANT'] = 'ID Магазина';
$MESS['OPTIONS_API_SECRET'] = 'Секретный ключ';
$MESS['OPTIONS_API_TEST_MODE'] = 'Тестовый режим';
$MESS['OPTIONS_API_CALLBACK'] = 'URL для возврата результатов от Мокка';
$MESS['OPTIONS_API_REDIRECT'] = 'URL для возврата пользователя после регистрации или оформления заказа';


